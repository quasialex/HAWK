...
if grep -q "^\\s\\* monitor" /tmp/hawk_iwinfo.txt; then
  ...
  if airmon-ng start "$IF"; then
    ...
    iw dev | sed -n "/Interface/,/txpower/p"
    echo "[i] You can now run airodump/wifite/etc on $MON_IF"
  else
  ...
  echo "    Common on brcmfmac/Broadcom 434xx. Options:"
  echo "    • Use external USB adapter that supports monitor/injection (e.g. AR9271/ath9k_htc, MT76xx)."
  echo "    • Or patch with nexmon (device-specific; not always stable)."
  echo ""
  echo "[*] Fallback: passive recon without monitor (iw scan every 5s, 12 cycles)."
  for i in $(seq 1 12); do
    echo "--- SCAN #$i ---"
    iw dev "$IF" scan 2>/dev/null | \
      awk \'/^BSS /{mac=$2}/SSID:/{ssid=$0; sub("SSID: ","",ssid)}/signal:/{sig=$2}/freq:/{f=$2} /^(\\t|$)/{if (mac!="" && ssid!=""){printf "BSS %s  sig %s dBm  freq %s  ssid %s\\n", mac, sig, f, ssid; mac=""; ssid=""; sig=""; f=""}}END{}\'
    sleep 5
  done
...
'}
      ...
      when 'psave_off'
        iface = p['iface'].to_s.empty? ? cfg['interfaces']['wifi'] : p['iface']
        cmd = %Q{bash -lc 'iw dev "#{iface}" set power_save off; iw dev "#{iface}" get power_save'}
        Hackberry::Exec.tmux_run(name:'iface', cmd: cmd, log_path: log)
